...
only install and run the one needed tool:
...
The server's required flags (`--output.text.path stderr --output.json.path stdout` for
...
v2.11.4) or was installed with a different major version. Run `install_tools` to
