# mcp-server-go-quality — Design Spec v2

**Date:** 2026-05-23
**Status:** draft
**Supersedes:** spec-v1.md

> **What changed from v1:** Resolved ambiguities A–M from the spec review. Key changes:
> hybrid per-tool timeout model (A), multi-module go.work support for nilaway (B),
> lazy install with version-aware caching (C), explicit config precedence hierarchy (D),
> corrected handler signatures (E), stderr-only install logging (F), pinned default tool
> versions (G), nilaway path-stripping fallback (H), standardised error string format (I),
> explicit top-level vs Diagnostic error decision rule (M), and corrected `extra_args`
> examples (K). Items J and L were confirmed consistent with v1 and carry forward unchanged.
>
> **v2-review changes (2026-05-23):** Timeout redefined as per-tool budget (not macro);
> `Severity` and `Column` fields added to `Diagnostic`; `install_tools` returns structured
> JSON with consistent object shapes across all three arrays; `run_code_checks` gains
> optional `tools` subset parameter; version-aware caching (resolved from `go version -m`,
> not requested specifier); root discovery walks up from `project_path`; NDJSON parse errors
> wrapped as JSON-encoded strings in `Native`; `--config` flag clarified as source-selector
> (still level 2); sentence boundary rule for nilaway messages; architecture diagram
> corrected; testdata minimum content specified including pinned vulnerable dep and call-graph
> requirement; Agent Workflow section added; per-request runner construction documented;
> pre-flight install scoped to only the tools about to run; govulncheck severity confirmed
> absent from Go vuln DB — always `""`; `latest` resolved once per process lifetime with
> air-gap fallback; `sync.RWMutex` on version cache; module paths mapping table added;
> `install_tools` gains optional `tools` subset parameter; error table includes build-error
> and tool-validation rows; `go.sum`-missing and max-response-size notes in AGENTS.md.
>
> **Resolved across 5 review rounds:** spec-v2-review.md through spec-v2-review5.md.
...
it calls the discovery subsystem directly to force-install all tools. It is not a checker.
...
Agent calls run_code_checks →
  ...
  Pre-flight: synchronous tool discovery (version-aware cache)
    → install any missing/wrong-version tools sequentially          ← avoids go install race
  ...
  Fire goroutines only for requested tools, each creating its own per-tool timeout:
    for _, tool := range requestedTools {
        go func() {
            toolCtx, cancel := context.WithTimeout(ctx, cfg.Timeout)
            defer cancel()
            // exec.Command under toolCtx → parse → send result to channel
        }()
    }
  Each handler:
    1. exec.Command under its independent toolCtx
    2. Parse tool's native JSON → extract Location/Severity/Message fields
    3. Return ([]Diagnostic, error) — timeout/error captured as Diagnostic
...
- `Message`: from `osv.summary`. If `osv.summary` is absent, use `osv.details` truncated to 120 characters. If both are absent, fall back to `finding.osv` ID.
...
1. On the **first incoming request from any check tool** (`run_code_checks`, `run_lint`,
   `run_vuln_check`, `run_nil_check`), discover each tool via `exec.LookPath`. The server
   reads the installed version via `go version -m` on the discovered binary path (see
   install_tools Output Format for the two-step Go pattern) and caches the resolved
   version (`toolName → v1.3.0`). When `.go-quality.yaml` requests a different version
   than the cached one, a re-install is triggered.
2. If any tool is missing (not on PATH) or has the wrong version, install all such tools
...
5. The `install_tools` MCP tool lets agents pre-install proactively before any analysis runs.
   ...
   all tools (or the requested `tools` subset), then updates only the matching cache entries
   with the newly resolved versions. After `install_tools` completes, the next check call
   has a warm cache and zero pre-flight overhead.
...
- `installed`: tools that were missing and now installed successfully.
- `already_present`: tools already on PATH with a matching version.
- `failed`: each entry includes the exact `command` that failed and its `stderr`.
...
If `go list -m -json <pkg>@latest` fails (e.g. network unavailable, private proxy
misconfigured), the server falls back to whatever is installed and sets the version
...
for _, check := range activeCheckers {
    ...
    go func(chk Checker) {
        ...
        result <- chk.Run(toolCtx, projectPath) // sends ([]Diagnostic, error) to a channel
    }(check)
}
...
No macro context is needed (there is only one checker running).
